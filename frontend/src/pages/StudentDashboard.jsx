  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 28 }}>
        <StatCard
          icon={CalendarDays} color="var(--blue)" bg="var(--blue-dim)"
          label="Upcoming Sessions" value={sessions.length}
        />
        <StatCard
          icon={ClipboardList} color="var(--green)" bg="var(--green-dim)"
          label="Tasks Pending" value={pending}
        />
        <StatCard
          icon={Flame} color="var(--amber)" bg="var(--amber-dim)"
          label="Current Streak" value="12" sub="days"
        />
        <StatCard
          icon={CheckCircle2} color="var(--teal)" bg="var(--teal-dim)"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
            ...
            <button style={{ fontSize: 12, color: 'var(--blue)', background: 'none', border: 'none', cursor: 'pointer' }}>View all</button>
          </div>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {tasks.slice(0, 5).map(t => {
              ...
              const isSoon = !isOverdue && due < new Date(Date.now() + 2 * 24 * 60 * 60 * 1000);
              return (
                <div key={t.id} style={{
